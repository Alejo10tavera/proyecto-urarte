...
if(count($routesArray) == 0){
	...
	if(count($routesArray) == 1 && isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] == "GET"){
		...
		if(isset($_GET["linkTo"]) && isset($_GET["equalTo"]) && !isset($_GET["rel"]) && !isset($_GET["type"])){
			...
			$response -> getFilterData(explode("?", $routesArray[1])[0], $_GET["linkTo"], $_GET["equalTo"], $_GET["linkTo_"], $_GET["equalTo_"], $orderBy, $orderMode, $startAt, $endAt, $_GET["select"]);
			...
			$response = new GetController();
			$response -> getRelFilterData($_GET["rel"], $_GET["type"], $_GET["linkTo"], $_GET["equalTo"], $_GET["linkTo_"], $_GET["equalTo_"], $orderBy, $orderMode, $startAt, $endAt, $_GET["select"]);
			...
			if( explode("?", $routesArray[1])[0] == "relations" && isset($_GET["rel"]) && isset($_GET["type"])){
				...
				$response = new GetController();
				$response -> getSearchRelData($_GET["rel"], $_GET["type"], $_GET["linkTo"], $_GET["linkTo_"], $_GET["equalTo_"], $_GET["search"], $orderBy, $orderMode, $startAt, $endAt, $_GET["select"]);

			}else{

				$response = new GetController();
				$response -> getSearchData(explode("?", $routesArray[1])[0], $_GET["linkTo"], $_GET["linkTo_"], $_GET["equalTo_"], $_GET["search"], $orderBy, $orderMode, $startAt, $endAt, $_GET["select"]);
			...
			if($count > 0){
				...
				=============================================*/	
					...
					if($_GET["token"] == "no"){
						...
						$user = GetModel::getFilterData("users", "token_user", $_GET["token"], "status_user", 1, null, null, null, null, "token_exp_user");
	...
	if(count($routesArray) == 1 && isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] == "PUT"){
		...
		if(isset($_GET["id"]) && isset($_GET["nameId"])){
			...
			$linkTo = $_GET["nameId"];
			$equalTo = $_GET["id"];
			$linkTo_ = $_GET["linkTo_"];
			$equalTo_ = $_GET["equalTo_"];
			$orderBy = null;
			$orderMode = null;
			$startAt = null;
			$endAt = null;
			...
			$response = PutController::getFilterData($table, $linkTo, $equalTo, $linkTo_, $equalTo_, $orderBy, $orderMode, $startAt, $endAt, $select);
			...
			if($response){
				...
				foreach (array_keys($data) as $key => $value) {
					...
					/*=============================================
					Agregamos excepción para actualizar las vistas de las tablas
					=============================================*/		
				...
				if($count > 0){
					...
					if(isset($_GET["token"])){
						...
						if($_GET["token"] == "no"){
							...
							$user = GetModel::getFilterData("users", "token_user", $_GET["token"], "bdelete_user", 0, null, null, null, null, "token_exp_user");
